          ...
          if(!nsContactId){
            ...
            if(accessToken && instanceUrl){
              ...
              if(instanceUrl){				
                ...
                instanceUrl = instanceUrl+"/services/data/v52.0/sobjects/Contact/" + extContactId +"?fields=MailingAddress";
                log.debug("1. Before sending request to SF", instanceUrl);
  ...
  function CreateSalesOrderRecord(objSalesOrder, transactionFrom){
    try{
      ...
      for(var prop in objSalesOrder) {
        ...
        //Set Main field value
        if(prop != "entity" && prop != "customer" && prop != "OpportunityContactRole" && prop != "OpportunityLineItem" && prop != "internalid" && prop != "forecasttype"){ 	
          try{
